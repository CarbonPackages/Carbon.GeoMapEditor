...
const neosifier = neos((globalRegistry) => ({
    ...
    config: globalRegistry
        ...
        .get("Carbon.GeoMapEditor"),
}));

const pointToFormatedString = ({ lat, lng }, f = 2) =>
    `${lat.toFixed(f)}" N, ${lng.toFixed(f)}" W`;
const pointToString = ({ lat, lng }) => `${lat.toFixed(7)},${lng.toFixed(7)}`;

function Editor(props) {
    const handleValueChange = (value) => props.commit(value);
    const [search, setSearch] = useState("");
    const { value, highlight, options, i18nRegistry, config, identifier } =
        props;
        ...
        <div className={clsx(style.editor, highlight && style.hightlight)}>
                ...
                <Fragment>
                    <label htmlFor={id} className={style.srOnly}>
                        {i18nRegistry.translate(
                            "Carbon.GeoMapEditor:Main:search",
                        )}
                    </label>
                    <TextInput
                        id={id}
            ...
            <div className={style.infoView}>
                <div className={style.propertyLabel}>
                    {hasValue ? pointToString(value) : ""}
                </div>
                <div className={style.propertyValue}>
                    {hasValue ? pointToFormatedString(value) : ""}
                </div>
            </div>
        </div>
    );
